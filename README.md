...
**Or skip editing CSS entirely** — the `THEME` dropdown switches the accent color live between 9 built-in presets (green/blue/orange/purple/red/yellow/pink/monochrome/retro terminal), and the `DARK MODE`/`LIGHT MODE` button flips the page background independently of accent choice. No restart needed for either. Both are global (like `REDUCE MOTION`/`AUTOSTART`) — one setting for every open screen, persisted in `dashboard/config.json`.

Under the hood this isn't 18 hand-picked palettes — `ACCENTS` in `dashboard/renderer/app.js` holds just *one* base hex per preset, and `MODES` holds the page background/text colors for dark vs. light plus which direction (toward black or toward white) the accent should mix to produce its "dim" (borders) and "faint" (hover backgrounds) shades, and its own glow formula (a diffuse additive bloom reads fine on a near-black page but is nearly invisible on a light one — light mode uses a tighter, higher-alpha halo plus a real drop shadow instead). `applyTheme(accent, mode)` computes the actual CSS variables at runtime via simple RGB lerping (`mixHex()`). Adding a 10th accent is one hex value in `ACCENTS` (plus a matching `<option>` in `index.html` and a name in `main.js`'s `THEME_NAMES` array, which validates preset names before writing them to disk) — no new dim/faint/glow shades to hand-tune.

`desktop-links`' icons are baked-pixel PNGs (recolored once at extraction time — see `desktop-links/extract-icon.ps1`), not something a CSS variable can reach directly. They still follow the active theme via a `hue-rotate()`/`saturate()` filter (`--icon-hue-rotate`/`--icon-saturate`, also set by `applyTheme()`) that rotates the baked-in green toward whatever accent's selected, and desaturates for monochrome — an approximation, not a true recolor, but close enough that icons don't stay stuck green under every other theme.
