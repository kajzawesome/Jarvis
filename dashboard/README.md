...
- **`THEME` dropdown** — switches the accent color live between 4 built-in presets (green/blue/amber/purple), no restart needed. Same global/broadcast/persist pattern as `REDUCE MOTION` (`theme-changed` event, `dashboard/config.json`). Implementation is just overriding `--green`/`--green-dim`/`--green-faint`/`--glow` on `:root` at runtime (`THEMES` table + `applyTheme()` in `renderer/app.js`) — no per-theme stylesheet to maintain, since every shared HUD class already reads those same 4 custom properties.
